...
<?php
		...
		$query=mysqli_query($con,"select * from `group` where member_id='$id' and group_stat='Active'")or die(mysqli_error($con));
  			while ($row=mysqli_fetch_array($query)){
  				$gid=$row['group_id'];

  				$list=mysqli_query($con,"select * from enrol natural join member where group_id='$gid' and status='approved'")or die(mysqli_error($con));
  						while ($rowlist=mysqli_fetch_array($list)){
?>															
			
						
											<div class="col-sm-12 col-md-6">
														<div class="thumbnail">
															<img src="../images/<?php echo $rowlist['member_pic'];?>" alt="100%x200" style="width: 100px;height:100px;display: block;" data-src="holder.js/100%x200">
															<div class="caption">
																<h5><?php echo $rowlist['member_first']." ".$rowlist['member_last'];?> </h5>
															</div>
														</div>
													</div>
<?php }}?>															
